...
- **言語**: Java 21 (開発・ビルド環境)
  - **実行環境互換性**: Java 7, 8, 17, 21 のすべてに対応すること。
  - **1.8.x 〜 1.16.x 対応**: Java 8 以降が必要。
  - **1.17.x 〜 1.20.4 対応**: Java 17 以降が必要。
  - **1.20.5 〜 1.21.x 対応**: Java 21 が必須。
...
### /paperchange (サーバーのバージョン変更)
- **実行コマンド**: `/paperchange` (Gemini CLI上で実行)
- **内容**: Paperサーバーのバージョンを対話形式で変更します。
- **対応バージョン**: 
  - 1.8 (1.8.8)
  - 1.12.2
  - 1.20.1 〜 1.20.6
  - 1.21.1 〜 1.21.11
- **仕組み**: 
  - ステップ1でメジャーバージョン（8, 12, 20, 21）を選択します。
  - ステップ2でマイナーバージョン（数字）を入力します。
  - `scripts/change_version.sh` が PaperMC API から最新ビルドを自動でダウンロードし、`run/paper.jar` を置き換えます。
- **注意**: バージョン変更後は、サーバーの再起動（StopしてからF5）が必要です。
